...
module TurboChat
  class ChatMembershipTest < ActiveSupport::TestCase
    test "role enum values" do
      ...
      chat = TurboChat::Chat.create!(title: "Roles")

      membership = TurboChat::ChatMembership.create!(chat: chat, participant: user, role: :moderator)
    ...
    test "pending invitation is inactive until accepted" do
      ...
      chat = TurboChat::Chat.create!(title: "Pending Membership")
    ...
    test "falls back to removed_at-only checks when invitation tracking column is unavailable" do
      ...
      chat = TurboChat::Chat.create!(title: "Legacy Membership")
      ...
      TurboChat::ChatMembership.stub(:invitation_tracking_supported?, false) do
        ...
        assert_equal [membership.id], TurboChat::ChatMembership.active.where(id: membership.id).pluck(:id)
        assert_empty TurboChat::ChatMembership.pending.where(id: membership.id)
      end
    end

    test "enforces max active participants per chat" do
      with_max_chat_participants(2) do
        chat = TurboChat::Chat.create!(title: "Participant Limit")
    ...
    test "removed memberships do not count but reactivating still checks the limit" do
      with_max_chat_participants(1) do
        ...
        first_membership = TurboChat::ChatMembership.create!(chat: chat, participant: first_user)
        second_membership = TurboChat::ChatMembership.create!(
          chat: chat,
    ...
    test "supports configured custom roles with name and permissions" do
      with_custom_role(
        ...
        chat = TurboChat::Chat.create!(title: "Custom Role")

        membership = TurboChat::ChatMembership.create!(chat: chat, participant: user, role_key: :support_agent)
    ...
    test "invalid custom role key is rejected" do
      ...
      chat = TurboChat::Chat.create!(title: "Invalid Role")

      membership = TurboChat::ChatMembership.new(chat: chat, participant: user, role_key: :unknown_role)
    ...
    def with_max_chat_participants(limit)
      previous_limit = TurboChat.configuration.max_chat_participants
      TurboChat.configuration.max_chat_participants = limit
      yield
    ensure
      TurboChat.configuration.max_chat_participants = previous_limit
    end
